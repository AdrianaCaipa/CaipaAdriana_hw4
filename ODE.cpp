...
//Componente en x
double dxdt2(double t, double x, double angulo,double x_dev){
	return 0-(c/m)*(c/m)*(x_dev*cos(angulo*PI/180));
}
//componente en y
double dydt2(double t, double y, double angulo, double y_dev){
	return -1.0/1.2*g-(c/m)*(y_dev*sin(angulo*PI/180));
}	
...
void rungeKutta_4(double *x, double *y, double *x_dev,double *y_dev, double h){
  ...
  kdx_1 = dxdt2(t,x_inicial,angulo,xdev_inicial);
  kdy_1 = dydt2(t,y_inicial,angulo,ydev_inicial);
  
  kx_2  = dxdt(t+h*0.5, x_inicial+kx_1*0.5, xdev_inicial+kdx_1*0.5);
  ky_2  = dydt(t+h*0.5, y_inicial+ky_1*0.5, ydev_inicial+kdy_1*0.5);
  kdx_2 = dxdt2(t+h*0.5, x_inicial+kx_1*0.5,angulo,xdev_inicial+kdx_1*0.5);
  kdy_2 = dydt2(t+h*0.5, y_inicial+ky_1*0.5,angulo,ydev_inicial+kdy_1*0.5);
		
  kx_3  = dxdt(t+h*0.5, x_inicial+kx_2*0.5, xdev_inicial+kdx_2*0.5);
  ky_3  = dydt(t+h*0.5, y_inicial+ky_2*0.5, ydev_inicial+kdy_2*0.5);
  kdx_3 = dxdt2(t+h*0.5, x_inicial+kx_2*0.5,angulo,xdev_inicial+kdx_2*0.5);
  kdy_3 = dydt2(t+h*0.5, y_inicial+ky_2*0.5,angulo,ydev_inicial+kdy_2*0.5);

  kx_4  = dxdt(t+h, x_inicial+kx_3,xdev_inicial+kdx_3);
  ky_4  = dydt(t+h, y_inicial+ky_3,ydev_inicial+kdy_3);
  kdx_4 = dxdt2(t+h, x_inicial+kx_3, angulo,xdev_inicial+kdy_3);
  kdy_4 = dydt2(t+h, y_inicial+ky_3,angulo, ydev_inicial+kdy_3);
